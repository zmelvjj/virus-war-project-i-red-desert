...
public static class BlazeUtils
{
    ...
    static ComputeShader s_ImageTransformShader;
    static int s_ImageSample = -1;
    ...
    static bool EnsureImageTransformShader()
    {
        if (s_ImageTransformShader != null && s_ImageSample >= 0)
            return true;

        s_ImageTransformShader = Resources.Load<ComputeShader>("ComputeShaders/ImageTransform");
        if (s_ImageTransformShader == null)
        {
            Debug.LogError("Missing Resources/ComputeShaders/ImageTransform.compute.");
            return false;
        }

        try
        {
            s_ImageSample = s_ImageTransformShader.FindKernel("ImageSample");
            return s_ImageSample >= 0;
        }
        catch (Exception e)
        {
            Debug.LogError($"ImageTransform.compute does not contain a valid ImageSample kernel: {e.Message}");
            s_ImageSample = -1;
            return false;
        }
    }
